import React from 'react';
import { Parallax, ParallaxLayer } from '@react-spring/parallax';
import TopBanner from './TopBanner';
import Image from 'next/image';

const BackgroundLayer: React.FC = () => {
  return (
    <div className="w-full h-full">


      <Parallax
        pages={6}
        innerStyle={{
          position: 'absolute',
          width: '100%',
          height: '100%',
          overflow: 'hidden',
        }}
        style={{
          top: '0',
          left: '0',
        }}>
        {/* Top Banner */}
        <ParallaxLayer offset={0} speed={1} style={{ zIndex: 1 }}>
          <div className="ml-56 mt-48 float-left">
            <Image
              src={"/assets/profile-picture-1.svg"}
              alt="Profile Picture"
              width={350}
              height={350}
              className="h-1/2"
            />
          </div>
        </ParallaxLayer>
        <ParallaxLayer offset={0} speed={1.8} style={{ zIndex: 1 }}>
          <div className="neobrutalist-title mr-96 mt-56 float-right">
            <h1 className="text-4xl font-bold">Jane Rukaria</h1>
          </div>
        </ParallaxLayer>
        <ParallaxLayer offset={0} speed={1.4} style={{ zIndex: 1 }}>
          <div className="neobrutalist-title mt-72 mr-56 w-auto float-right">
            <h2 className="text-2xl ">Lorem ipsum dolor sit amet</h2>
          </div>
        </ParallaxLayer>
        <ParallaxLayer offset={0} speed={1} style={{ zIndex: 1 }}>
          <div className="neobrutalist-title mt-96 mr-56 float-right w-96">
            <h2 className="text-2xl ">Mustache put a bird on it franzen kinfolk beard, street art exercitation sartorial marxism pop-up enamel pin distillery.</h2>
          </div>
        </ParallaxLayer>
        <ParallaxLayer offset={0.5} speed={1.5}>
          <svg viewBox="0 0 540 960" width="100vw">
            <g transform="translate(231.04974937196394 513.4441600811562)"><path d="M157.1 -288.3C201.6 -246.5 234.2 -200.2 271.2 -151.5C308.2 -102.7 349.6 -51.3 349.4 -0.1C349.3 51.2 307.6 102.3 267.2 145.2C226.7 188.1 187.6 222.6 143.3 248.7C99 274.8 49.5 292.4 -5 301.1C-59.5 309.7 -119 309.4 -155.5 278.9C-192.1 248.3 -205.6 187.4 -219.1 135.9C-232.5 84.4 -245.7 42.2 -257.1 -6.6C-268.5 -55.3 -278 -110.7 -265.8 -164.2C-253.5 -217.8 -219.5 -269.5 -171.6 -309.4C-123.7 -349.2 -61.8 -377.1 -2.8 -372.3C56.3 -367.6 112.7 -330.1 157.1 -288.3" fill="#B42518"></path></g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={0.5} speed={1}>
          <svg viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(292.9359697381952 458.5490223491531)"><path d="M164.4 -282.9C205.1 -261.2 224.6 -201.1 248.9 -147.5C273.1 -94 302.1 -47 304.1 1.2C306.1 49.3 281.2 98.7 256.3 151.1C231.4 203.5 206.4 258.9 163.7 293.9C121 328.9 60.5 343.5 6.2 332.8C-48.2 322.1 -96.3 286.2 -139.3 251.4C-182.4 216.5 -220.2 182.8 -263.6 141C-307.1 99.3 -356 49.7 -349.5 3.7C-343.1 -42.2 -281.1 -84.4 -237.6 -125.8C-194 -167.2 -168.7 -207.9 -132.2 -232C-95.7 -256 -47.8 -263.5 7 -275.6C61.8 -287.8 123.7 -304.5 164.4 -282.9" fill="#C5291B"></path></g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={0.5} speed={1.5}>
          <svg viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(283.7689565993613 450.0126535710416)"><path d="M147.1 -263.8C188.3 -231.1 217.5 -186.8 251.7 -140.9C285.9 -95 324.9 -47.5 321.2 -2.2C317.4 43.2 270.9 86.3 244 144.8C217 203.2 209.8 277 172.4 320.4C135 363.8 67.5 376.9 10.1 359.4C-47.3 342 -94.7 294 -142.9 256.8C-191.1 219.6 -240.2 193.3 -276.5 152.3C-312.8 111.3 -336.4 55.7 -345.4 -5.2C-354.3 -66 -348.6 -132 -305.5 -161.2C-262.4 -190.5 -181.8 -182.9 -124.7 -206.4C-67.5 -229.9 -33.8 -284.5 9.6 -301.1C53 -317.8 106 -296.6 147.1 -263.8" fill="#D72D1D"></path></g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={0.5} speed={2}>
          <svg viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(301.34094405344865 521.1254901021773)"><path d="M99.3 -205.7C128.6 -155.1 152.2 -128.3 172 -98C191.9 -67.7 207.9 -33.8 211 1.7C214 37.3 204 74.7 185.1 106.6C166.2 138.5 138.3 164.9 106 180C73.7 195.1 36.8 198.8 -13.9 222.9C-64.7 247 -129.3 291.5 -180.6 287.3C-231.8 283.2 -269.6 230.3 -273.8 174.3C-278 118.3 -248.5 59.2 -242.3 3.6C-236.1 -52 -253.1 -104 -249.7 -161.3C-246.3 -218.6 -222.4 -281.2 -177.4 -322.7C-132.3 -364.2 -66.2 -384.6 -15.6 -357.6C35 -330.6 70 -256.2 99.3 -205.7" fill="#e23728"></path></g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={0.5} speed={2.5}>
          <svg viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(256.62084825365855 518.5420700137327)"><path d="M169.2 -307.9C203.4 -273.3 204.3 -195.9 204.3 -137.5C204.3 -79 203.4 -39.5 218.7 8.8C234 57.2 265.5 114.3 250.1 146.1C234.7 177.8 172.3 184.2 122.8 192.9C73.3 201.7 36.7 212.8 -9 228.4C-54.7 244 -109.3 264 -145.7 247.7C-182.1 231.4 -200.2 178.7 -210.8 131.3C-221.5 84 -224.7 42 -226.6 -1.1C-228.5 -44.2 -229 -88.3 -209.6 -120.5C-190.2 -152.7 -150.8 -172.9 -112.6 -205.2C-74.3 -237.4 -37.2 -281.7 15.2 -308C67.5 -334.2 135 -342.5 169.2 -307.9" fill="#e4483a"></path></g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={2} speed={0.5}>
          <svg viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(276.4998444715307 452.88218852540905)">
              <path d="M149.7 -244.7C187 -237.9 205.2 -183.4 214.6 -134.7C224 -86 224.5 -43 206.6 -10.3C188.7 22.3 152.4 44.7 142.5 92.6C132.7 140.5 149.3 214 130 255.3C110.7 296.7 55.3 305.8 5.6 296.2C-44.2 286.5 -88.3 258 -106.6 216C-124.9 174.1 -117.2 118.7 -117.5 80.4C-117.7 42.2 -125.9 21.1 -156.1 -17.5C-186.3 -56 -238.7 -112 -233.7 -142.2C-228.8 -172.3 -166.7 -176.7 -118.2 -177C-69.7 -177.3 -34.8 -173.7 10.7 -192.2C56.2 -210.8 112.5 -251.5 149.7 -244.7" fill="#DC7B04"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={2} speed={1}>
          <svg id="visual" viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(243.50219665480938 474.94553381584706)">
              <path
                d="M114.6 -222.2C132.9 -187.9 121.2 -125.6 152.2 -83.9C183.1 -42.2 256.5 -21.1 280.4 13.8C304.3 48.7 278.6 97.3 238 122.3C197.4 147.3 142 148.5 99.8 179.5C57.7 210.5 28.8 271.3 -6.3 282.2C-41.5 293.2 -83 254.4 -131.7 227.2C-180.4 200 -236.4 184.4 -236.6 148.4C-236.9 112.5 -181.4 56.3 -164.8 9.6C-148.1 -37 -170.2 -74 -176.8 -121.8C-183.3 -169.6 -174.4 -228.1 -142.4 -254.4C-110.3 -280.8 -55.2 -274.9 -3.5 -268.8C48.2 -262.8 96.3 -256.5 114.6 -222.2"
                fill="#F18805"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={2} speed={1.5}>
          <svg id="visual" viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(263.3202421470379 481.7822194339665)">
              <path
                d="M89.7 -142.2C119.5 -138.3 148.9 -120.6 190.2 -94.6C231.4 -68.7 284.5 -34.3 294.1 5.6C303.8 45.5 270.1 91 226 112.1C181.9 133.1 127.5 129.8 88.1 130.5C48.7 131.3 24.3 136.1 -11.6 156.2C-47.5 176.3 -95 211.5 -141.3 214.8C-187.7 218 -232.8 189.3 -258.4 148.1C-284 107 -290 53.5 -272.2 10.3C-254.5 -33 -213 -66 -189.7 -111.2C-166.5 -156.4 -161.5 -213.7 -132.9 -218.3C-104.3 -222.9 -52.2 -174.7 -11.1 -155.5C30 -136.3 60 -146.1 89.7 -142.2"
                fill="#FA900F"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={2} speed={2}>
          <svg id="visual" viewBox="0 0 540 960" width="70vw" >
            <g transform="translate(281.1784443548942 531.4973398686511)">
              <path
                d="M104.7 -184C151 -154.5 214.5 -157.5 221 -132.3C227.5 -107 177 -53.5 164.6 -7.2C152.2 39.2 177.9 78.3 170.5 102.1C163.2 126 122.8 134.4 88.9 147.8C55 161.3 27.5 179.6 -4.4 187.3C-36.3 194.9 -72.7 191.9 -101.1 175.3C-129.5 158.7 -150.1 128.6 -180.4 97.1C-210.7 65.7 -250.9 32.8 -242.9 4.6C-235 -23.7 -179 -47.3 -153.7 -87.6C-128.5 -127.9 -134 -184.7 -113.5 -229.2C-93 -273.6 -46.5 -305.5 -8.7 -290.5C29.2 -275.5 58.3 -213.5 104.7 -184"
                fill="#FB9A23"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={2} speed={2.5}>
          <svg id="visual" viewBox="0 0 540 960" width="60vw" >
            <g transform="translate(286.9861950452828 449.2249253315797)">
              <path
                d="M90.9 -142.9C121.8 -139.6 153.6 -123.3 185.1 -97.3C216.6 -71.3 247.8 -35.7 258.6 6.2C269.4 48.2 259.9 96.3 240.5 143.4C221.2 190.4 192.1 236.4 150.4 250.5C108.7 264.5 54.3 246.8 8.3 232.3C-37.7 217.9 -75.3 206.8 -95.3 180.1C-115.2 153.5 -117.3 111.2 -151.2 78.6C-185 46 -250.5 23 -279.3 -16.6C-308.1 -56.2 -300.2 -112.5 -268.2 -148.3C-236.2 -184.1 -180.1 -199.4 -131.4 -192.4C-82.7 -185.4 -41.3 -156 -5.7 -146.2C30 -136.3 60 -146.1 90.9 -142.9"
                fill="#FBA337"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={2} speed={3}>
          <svg id="visual" viewBox="0 0 540 960" width="50vw" >
            <g transform="translate(275.8053069283999 498.94230914695316)">
              <path
                d="M146.9 -251.9C181.2 -234.5 193.7 -176.8 203.2 -128C212.7 -79.3 219.4 -39.7 217.1 -1.3C214.8 37 203.5 74 184.7 106.6C165.8 139.2 139.4 167.5 107.4 182.8C75.3 198.1 37.7 200.6 -4.7 208.7C-47 216.7 -94 230.5 -112.3 207.2C-130.5 183.9 -120.1 123.6 -131.1 82.9C-142.1 42.2 -174.5 21.1 -198.8 -14C-223.2 -49.2 -239.3 -98.3 -221.8 -127.8C-204.3 -157.2 -153.2 -166.9 -110.6 -179.5C-68 -192.1 -34 -207.6 11.1 -226.8C56.2 -246.1 112.5 -269.2 146.9 -251.9"
                fill="#FBAC4B"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={4} speed={0.5}>
          <svg id="visual" viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(260.34602362600526 539.4975759524089)">
              <path
                d="M158.6 -312.3C184.6 -259.8 170.3 -174.9 192.9 -117.4C215.6 -60 275.3 -30 278.2 1.7C281.1 33.3 227.1 66.7 200.9 117.9C174.6 169.1 176.1 238.3 147.2 255.1C118.3 272 59.2 236.5 16.9 207.2C-25.3 177.8 -50.6 154.7 -96.7 147.8C-142.7 140.9 -209.5 150.2 -239.2 128.3C-268.8 106.3 -261.4 53.2 -241.9 11.3C-222.4 -30.7 -190.9 -61.3 -177 -110.6C-163.1 -159.8 -166.8 -227.5 -140.2 -279.7C-113.7 -331.9 -56.8 -368.4 4.7 -376.7C66.3 -384.9 132.7 -364.8 158.6 -312.3"
                fill="#235214"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={4} speed={1}>
          <svg id="visual" viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(349.9819928198464 448.614805912849)">
              <path
                d="M104.2 -196.2C135.1 -162.7 160.2 -134.8 167.2 -103.1C174.2 -71.3 163 -35.7 172.6 5.5C182.1 46.7 212.3 93.3 211.2 135.3C210.2 177.3 177.8 214.7 137.4 245.7C97 276.7 48.5 301.3 -9.5 317.8C-67.5 334.2 -135 342.5 -174.7 311.1C-214.4 279.7 -226.3 208.6 -259.7 150.1C-293.1 91.7 -348.1 45.8 -365.2 -9.9C-382.4 -65.7 -361.8 -131.3 -313.4 -163.8C-265 -196.3 -188.7 -195.6 -131.9 -214.1C-75 -232.6 -37.5 -270.3 -0.4 -269.6C36.7 -268.8 73.3 -229.7 104.2 -196.2"
                fill="#2A6419"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={4} speed={1.5}>
          <svg id="visual" viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(284.69382887354794 522.2553507911193)">
              <path
                d="M130.8 -258.6C149 -216 129.3 -139.6 131.2 -90.9C133.1 -42.2 156.5 -21.1 166.4 5.7C176.3 32.5 172.6 65 163.3 100.9C154.1 136.9 139.3 176.3 111.1 196.7C83 217.1 41.5 218.5 -2 222C-45.5 225.5 -91 230.9 -112.8 206.9C-134.6 182.8 -132.7 129.1 -148.6 89.7C-164.5 50.3 -198.3 25.2 -201.4 -1.8C-204.6 -28.8 -177.2 -57.7 -166.4 -105.9C-155.5 -154.1 -161.3 -221.7 -136.3 -260.4C-111.3 -299.2 -55.7 -309.1 0.3 -309.6C56.2 -310.1 112.5 -301.2 130.8 -258.6"
                fill="#378321"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={4} speed={2}>
          <svg id="visual" viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(272.7577153039157 535.5091651599273)">
              <path
                d="M94.9 -194.5C113.2 -153.9 111.4 -108.6 139.1 -75.4C166.7 -42.2 223.9 -21.1 235.8 6.9C247.7 34.8 214.3 69.7 184 98.2C153.7 126.8 126.3 149.1 96.2 155.4C66 161.6 33 151.8 5.3 142.6C-22.3 133.3 -44.7 124.7 -93.4 129.2C-142.1 133.6 -217.2 151.2 -237.5 131.8C-257.9 112.5 -223.4 56.3 -208.2 8.8C-193 -38.7 -196.9 -77.3 -182.9 -107.6C-168.9 -138 -137 -159.9 -103.5 -191.7C-70 -223.6 -35 -265.3 1.7 -268.2C38.3 -271.1 76.7 -235.1 94.9 -194.5"
                fill="#3D9325"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={4} speed={2.5}>
          <svg id="visual" viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(273.90753714735223 463.0854253414292)">
              <path
                d="M126 -214.4C152 -203.2 153.9 -146.6 160.4 -103.3C166.9 -60 178 -30 177.4 -0.3C176.8 29.3 164.6 58.7 145.4 79.9C126.1 101.1 99.8 114.2 74.4 137.2C49 160.2 24.5 193.1 -15.9 220.6C-56.2 248.1 -112.5 270.2 -146 251.9C-179.5 233.5 -190.2 174.8 -183.2 126C-176.1 77.3 -151.4 38.7 -141.5 5.7C-131.7 -27.3 -136.9 -54.7 -142.1 -100.3C-147.4 -145.9 -152.7 -209.8 -129 -222.3C-105.3 -234.8 -52.7 -195.9 -1.3 -193.6C50 -191.3 100 -225.5 126 -214.4"
                fill="#43A329"></path>
            </g>
          </svg>
        </ParallaxLayer>
        <ParallaxLayer offset={4} speed={3}>
          <svg id="visual" viewBox="0 0 540 960" width="100vw" >
            <g transform="translate(285.68069038356634 430.1029093268936)">
              <path
                d="M79.3 -147.1C105.5 -122.1 131.6 -106.6 156 -83.6C180.4 -60.7 203.2 -30.3 214 6.2C224.9 42.8 223.7 85.7 211.2 129.3C198.7 172.8 174.9 217.2 137.8 252C100.7 286.9 50.3 312.2 8.1 298.2C-34.2 284.2 -68.3 230.9 -86.7 185.2C-105 139.5 -107.5 101.5 -144.4 71.9C-181.3 42.3 -252.7 21.2 -252.8 -0.1C-253 -21.3 -181.9 -42.7 -138.4 -60.9C-95 -79.2 -79.1 -94.4 -60.7 -123.9C-42.2 -153.4 -21.1 -197.2 2.7 -201.9C26.5 -206.6 53 -172.1 79.3 -147.1"
                fill="#4AB42D"></path>
            </g>
          </svg>
        </ParallaxLayer>
      </Parallax>
    </div>
  );
};

export default BackgroundLayer;